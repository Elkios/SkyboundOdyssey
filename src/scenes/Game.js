...
class GameScene extends Phaser.Scene {
  ...
  preload() {
    ...
    // load flying die
    this.load.spritesheet(
      "character_flying_die",
      "assets/player/player_flying_weapon_die.png",
      {
        // 4405×639
        frameWidth: 4405 / 5,
        frameHeight: 639 / 1,
    ...
    this.load.spritesheet(
      "character_idle_die",
      "assets/player/player_standing_idle_weapon_die.png",
      {
        // 4405×639
        frameWidth: 4405 / 5,
        frameHeight: 639 / 1,
  ...
  create() {
    ...
    this.anims.create({
      key: "flying_die",
      frames: this.anims.generateFrameNumbers("character_flying_die", {
        start: 0,
        end: 4,
      }),
      frameRate: 10,
    ...
    this.anims.create({
      key: "idle_die",
      frames: this.anims.generateFrameNumbers("character_idle_die", {
        start: 0,
        end: 4,
      }),
      frameRate: 10,
    ...
    const distanceBg = this.add.image(
    ...
    this.distanceText = this.add.text(10, 10, "Distance: 0 m", {
      font: "18px Arial",
      color: "#ffffff",
    });
    ...
    Phaser.Display.Align.In.Center(this.distanceText, distanceBg);
    // Obstacles
  ...
  gameOver() {
    ...
    const isOnFloor =
      this.character.body.blocked.down || this.character.body.touching.down;
    if (!isOnFloor) {
  ...
  showGameOverScreen() {
    ...
    const gameOverText = `Game Over\nDistance parcourue : ${Math.floor(
      this.distance
    )} m`;
    this.add
      .text(this.scale.width / 2, this.scale.height / 2, gameOverText, {
        fontSize: "32px",
        fill: "#000",
        align: "center",
      })
    ...
    // Add a revive button
    const reviveButton = this.add
      .text(this.scale.width / 2, this.scale.height / 2 + 100, "Recommencer", {
        fontSize: "32px",
        fill: "#FFF",
        align: "center",
      })
    ...
    // Center the text in the button
    Phaser.Display.Align.In.Center(reviveButton, buttonImage);
    ...
    // Add button image to the button
    const menuButtonImage = this.add.image(
      this.scale.width / 2,
    ...
    // Center the text in the button
    Phaser.Display.Align.In.Center(menuButton, menuButtonImage);

    // Add a button to save the score
    const saveScoreButtonImage = this.add.image(
      this.scale.width / 2,
    ...
    const saveScoreButton = this.add
      .text(
        this.scale.width / 2,
        this.scale.height / 2 + 300,
    ...
    saveScoreButton.on(
      ...
      async () => {
        ...
        if (playerName && playerName.trim() !== "") {
          ...
          try {
            await this.dbCRUD.setRank(
              playerName.trim(),
              Math.floor(this.distance),
              this.patternManager.getCoins()
            );
          } catch (e) {
            console.error(e);
          } finally {
  ...
  update(time, delta) {
    ...
    if (this.isDead) {
      ...
      if (isOnFloor && velocityY <= 0) {
        ...
        this.character.setVelocityX(0);
        this.character.anims.play("idle_die", true).on(
          "animationcomplete",
          () => {
            // Disable collisions
